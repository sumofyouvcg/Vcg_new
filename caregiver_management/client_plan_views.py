...
from vcg.company_management.models import CompanyClientPlan, ClientCaregivers
...
def client_plan_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('caregiver/client_plan_list.html', locals(), context_instance=RequestContext(request))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(request,(_("Practise Plan Saved Successfully")),
                                         fail_silently = True)
            return redirect(reverse('caregiver_client_plan_list', kwargs={'domain_name': domain_name, 'client_id':str(client_id)}))
        else:
            return render_to_response('caregiver/client_plan_add.html', locals(), context_instance = RequestContext(request))
    else:
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                plan.assign_status = True
                plan.save()
                client_caregiver = ClientCaregivers.objects.filter(client = plan.client , caregiver = plan.caregiver, created_by = request.user, modified_by = request.user)
                ...
                return render_to_response('caregiver/client_plan_edit.html', locals(), context_instance=RequestContext(request))
        else:
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_plan_view.html', locals(), context_instance = RequestContext(request))
