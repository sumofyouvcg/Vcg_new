...
from vcg.admin_management.models import Company, Caregiver
...
def chat_user(request, domain_name = None):
    ...
    state=''
    return render_to_response('caregiver/caregiver_chat.html', locals(), context_instance=RequestContext(request))
...
def post(request, domain_name = None):
    ...
    if not request.is_ajax():
        HttpResponse (_("Not an AJAX request"))
    ...
    return HttpResponse (_("Not an POST request"))
...
def get(request, domain_name = None):
    ...
    print "=" * 50
    if not request.is_ajax():
        return  HttpResponse (_("Not an AJAX request")) 
    ...
    print "========= After Scaling ==============="
    if Caregiver.objects.filter(caregiver_number = request.user.username):

        caregiver    = Caregiver.objects.get(caregiver_number = request.user.username)
#Caregiver-Clients        
        clients    = ClientCaregivers.objects.filter(caregiver = caregiver, caregiver__role__in = ['Company','Analyst'])
...
#Caregiver-Caregivers        
        ...
        for caregiver in caregiver_caregivers:
            if not request.user.username == caregiver.caregiver_number:
                caregiver_caregivers_list.append(caregiver.caregiver_number)
                
    last_active_list = LastActive.objects.filter((Q(user__username__in = caregiver_client_list)|Q(user__username = caregiver.company.company_number)|Q(user__username__in = caregiver_caregivers_list)), received_at__gt = datetime.datetime.now() - datetime.timedelta(seconds=29090))
    users_list = []
    for last_active in last_active_list:
        users_list.append(last_active.user.username)
...
@caregiver_login
def caregiver_chat_details(request, domain_name = None):
    ...
    return render_to_response('caregiver/caregiver_chat_details.html', locals(), context_instance = RequestContext(request))

@caregiver_login
def caregiver_chat_persons(request, domain_name = None, date = None):
    message_list = Messages.objects.filter(recipient = request.user, delete_status = False).order_by('-modified_at')
    new_messages = Messages.objects.filter(recipient = request.user, read_status = False, delete_status = False)
    ...
    return render_to_response('caregiver/caregiver_chat_persons.html', locals(), context_instance = RequestContext(request))

@caregiver_login
def caregiver_chat_history(request, domain_name = None, date = None, user_id = None):
    message_list = Messages.objects.filter(recipient = request.user, delete_status = False).order_by('-modified_at')
    new_messages = Messages.objects.filter(recipient = request.user, read_status = False, delete_status = False)
    ...
    return render_to_response('caregiver/caregiver_chat_history.html', locals(), context_instance = RequestContext(request))