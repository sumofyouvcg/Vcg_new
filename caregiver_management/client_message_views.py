...
def client_send_message(request, domain_name = None, client_id = None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(reverse('caregiver_client_home', kwargs={'domain_name': domain_name, 'client_id':str(client_id)}))
        else:
            return render_to_response('caregiver/client_send_message.html', locals(), context_instance = RequestContext(request))
    else:
...
def client_messages(request, domain_name = None, client_id = None):
    ...
    return render_to_response('caregiver/client_messages.html', locals(), context_instance=RequestContext(request))
...
def client_messages_delete(request, domain_name = None, client_id = None):
    ...
    return redirect(reverse('caregiver_client_messages', kwargs={'domain_name': domain_name, 'client_id':str(client_id)})) 
...
def client_view_message(request, domain_name = None, client_id = None, msg_id = None):
    try:
        ...
        client_user = User.objects.get(username = client.client_number)
        new_messages  = len(Messages.objects.filter(created_by = client_user, recipient = request.user, read_status = False))
        return render_to_response('caregiver/client_view_message.html', locals(), context_instance=RequestContext(request))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_reply_message.html', locals(), context_instance = RequestContext(request))
        else:
            form = MessagesForm(user = request.user) 
            return render_to_response('caregiver/client_reply_message.html', locals(), context_instance=RequestContext(request))
    except Messages.DoesNotExist:
