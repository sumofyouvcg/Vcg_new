...
def client_intake_form(request, domain_name = None, client_id = None, form_id = None):
    ...
    try:
        ...
        if client_intake:
            return redirect(reverse('caregiver_client_intake_form_completed', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id':form_id, 'intake_id': client_intake[0].id}))
        else:
            form1 = ClientIntakeFormForm1()
            form2 = ClientIntakeFormForm2()
            form3 = ClientIntakeFormForm3()
        return render_to_response('caregiver/client_intake_form.html', locals(), context_instance=RequestContext(request))
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_intake_form_completed.html', locals(), context_instance=RequestContext(request))   
        else:
            return render_to_response('caregiver/client_intake_form_completed.html', locals(), context_instance=RequestContext(request))   
