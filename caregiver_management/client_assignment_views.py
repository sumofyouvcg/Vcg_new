...
def client_assignment_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('caregiver/client_assignment_list.html', locals(), context_instance=RequestContext(request))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(request,(_("Assignment Assigned Successfully")),
                                         fail_silently = True)
            return redirect(reverse('caregiver_client_assignment_list', kwargs={'domain_name': domain_name, 'client_id':client_id}))
        else:
            return render_to_response('caregiver/client_assignment_add.html', locals(), context_instance = RequestContext(request))
    else:
...
def client_assignment_view(request, domain_name = None, client_id = None, assign_id = None):
    ...
    try:
        ...
        return render_to_response('caregiver/client_assignment_view.html', locals(), context_instance=RequestContext(request))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_assignment_details.html', locals(), context_instance = RequestContext(request))
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_assignment_edit.html', locals(), context_instance=RequestContext(request))
        else:
...
def client_assignment_questions(request, domain_name = None, client_id = None, assign_id = None, cluster_id = None):
    ...
    try:
        ...
        try:
            ...
            assignment_questions = paginate.page(paginate.num_pages)
        num_pages = range(assignment_questions.paginator.num_pages)
        return render_to_response('caregiver/client_assignment_questions.html', locals(), context_instance=RequestContext(request))
    except CompanyClientAssignment.DoesNotExist:
        messages.success(request,(_('This Assignment has been removed by Admin')), fail_silently= True)
        return redirect(reverse('caregiver_client_assignment_list', kwargs={'domain_name': domain_name, 'client_id':client_id}))
