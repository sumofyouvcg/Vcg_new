...
def client_treatment_agreement_form(request, domain_name = None, client_id = None, form_id = None):
    ...
    try:
        ...
        if client_treatment_agreement:
            return redirect(reverse('caregiver_client_treatment_agreement_form_completed', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id':form_id, 'treat_agree_id': client_treatment_agreement[0].id}))
        else:
            form = TreatmentAgreementForm()
        return render_to_response('caregiver/client_treatment_agreement_form.html', locals(), context_instance=RequestContext(request))
    ...
    try:
        ...
        if request.POST:
            ...
            if form1.is_valid():
                ...
                return render_to_response('caregiver/client_treatment_agreement_form_completed.html', locals(), context_instance=RequestContext(request))   
        else:
            return render_to_response('caregiver/client_treatment_agreement_form_completed.html', locals(), context_instance=RequestContext(request))   
