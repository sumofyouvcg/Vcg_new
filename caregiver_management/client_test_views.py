...
from vcg.admin_management.models import Client, Caregiver, TestQuestion, TestRange, TestQuestionAnswers
...
def client_test_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('caregiver/client_test_list.html', locals(), context_instance=RequestContext(request))
...
def client_test_add(request, domain_name = None, client_id = None):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            test.save()
            ClientCaregivers.objects.create(client = test.client , caregiver = test.caregiver, task_id = "test_"+str(test.id))
            messages.success(request,(_("Test Assigned Successfully")),
                                         fail_silently = True)
            return redirect(reverse('caregiver_client_test_list', kwargs={'domain_name': domain_name, 'client_id':str(client_id)}))
        else:
            return render_to_response('caregiver/client_test_add.html', locals(), context_instance = RequestContext(request))
    else:
...
def client_test_view(request, domain_name = None, client_id = None, test_id = None):
    ...
    try:
        ...
        if test_result:
            ...
            return render_to_response('caregiver/client_test_completed.html', locals(), context_instance=RequestContext(request))
        return render_to_response('caregiver/client_test_view.html', locals(), context_instance=RequestContext(request))
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_test_edit.html', locals(), context_instance=RequestContext(request))
        else:
...
def client_test_feedback(request, domain_name = None, client_id = None, test_id = None):
    ...
    try:
        ...
        messages.success(request,(_('This Test has been removed by Admin')), fail_silently= True)
        return redirect('/caregiver_management/client_test_list/'+client_id+'/')
