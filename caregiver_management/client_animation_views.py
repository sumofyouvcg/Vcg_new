...
def client_animation_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('caregiver/client_animation_list.html', locals(), context_instance=RequestContext(request))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(request,(_("Animation Saved Successfully")),
                                         fail_silently = True)
            return redirect(reverse('caregiver_client_animation_list', kwargs={'domain_name': domain_name, 'client_id':str(client_id)}))
        else:
            return render_to_response('caregiver/client_animation_add.html', locals(), context_instance = RequestContext(request))
    else:
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                animation.assign_status = True
                animation.save()
                client_caregiver = ClientCaregivers.objects.filter(client = animation.client , caregiver = animation.caregiver, created_by = request.user, modified_by = request.user)
                ...
                return render_to_response('caregiver/client_animation_edit.html', locals(), context_instance=RequestContext(request))
        else:
            form = CompanyClientAnimationForm(user=request.user, instance = selected_animation)
        return render_to_response('caregiver/client_animation_edit.html', locals(), context_instance=RequestContext(request))
    except CompanyClientAnimation.DoesNotExist:
        messages.success(request,('This Animation has been removed by Admin'), fail_silently= True)
        return redirect(reverse('caregiver_client_animation_list', kwargs={'domain_name': domain_name, 'client_id':client_id}))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('caregiver/client_animation_view.html', locals(), context_instance = RequestContext(request))
