...
@company_login
def company_message(request, domain_name = None):
    ...
    return render_to_response('company/company_message.html', locals(), context_instance=RequestContext(request))

@company_login
def company_sent_message(request, domain_name = None):
    ...
    try:
        ...
        message_list = paginate.page(paginate.num_pages)
    num_pages = range(message_list.paginator.num_pages)    
    return render_to_response('company/company_sent_message.html', locals(), context_instance=RequestContext(request))

@company_login
def company_trash_message(request, domain_name = None):
    ...
    try:
        ...
        message_list = paginate.page(paginate.num_pages)
    num_pages = range(message_list.paginator.num_pages)    
    return render_to_response('company/company_trash_message.html', locals(), context_instance=RequestContext(request))

@company_login
def company_message_delete(request, domain_name = None):
    ...
    return redirect(reverse('company_message', kwargs={'domain_name': domain_name})) 


@company_login
def company_sent_message_delete(request, domain_name = None):
    if request.method == 'POST':
        message_ids    = request.POST.getlist('choices')
        for message_id in message_ids:
    ...
    return redirect(reverse('company_sent_message', kwargs={'domain_name': domain_name}))  


@company_login
def company_trash_message_delete(request, domain_name = None):
    if request.method == 'POST':
        message_ids    = request.POST.getlist('choices')
        for message_id in message_ids:
        ...
        if len(message_ids) > 1:
            ...
            messages.success(request,(_("The Messages has been Deleted Successfully")),fail_silently = True)
    return redirect(reverse('company_trash_message', kwargs={'domain_name': domain_name}))  

@company_login
def company_move_to_inbox(request, domain_name = None):
    if request.method == 'POST':
        message_ids    = request.POST.getlist('choices')
        for message_id in message_ids:
...
def view_message(request, domain_name = None, client_id = None, msg_id = None):
    ...
    try:
        ...
        return redirect(reverse('company_message', kwargs={'domain_name': domain_name})) 
    return render_to_response('company/view_message.html', locals(), context_instance=RequestContext(request))
...
def view_sent_message(request, domain_name = None, client_id = None, msg_id = None):
    ...
    try:
        ...
        messages.success(request,(_('This Message has been removed')), fail_silently= True)
        return redirect(reverse('company_sent_message', kwargs={'domain_name': domain_name}))  
    return render_to_response('company/view_sent_message.html', locals(), context_instance=RequestContext(request))
...
def view_trash_message(request, domain_name = None, client_id = None, msg_id = None):
    ...
    try:
        ...
        messages.success(request,(_('This Message has been removed')), fail_silently= True)
        return redirect(reverse('company_trash_message', kwargs={'domain_name': domain_name}))  
    return render_to_response('company/view_trash_message.html', locals(), context_instance=RequestContext(request))
...
def new_message(request, domain_name = None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render_to_response('company/new_message.html', locals(), context_instance = RequestContext(request))
    else:
...
def reply_message(request, domain_name = None, reply_id= None):
    new_messages = Messages.objects.filter(recipient = request.user, read_status = False, delete_status = False)
    try:
        msg = Messages.objects.get(id = reply_id)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('company/reply_message.html', locals(), context_instance = RequestContext(request))
        else:
            form = MessagesForm(user=request.user) 
        return render_to_response('company/reply_message.html', locals(), context_instance=RequestContext(request))
    except Messages.DoesNotExist:
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('company/reply_trash_message.html', locals(), context_instance = RequestContext(request))
        else:
            form = MessagesForm(user=request.user) 
        return render_to_response('company/reply_trash_message.html', locals(), context_instance=RequestContext(request))
    except Messages.DoesNotExist:
