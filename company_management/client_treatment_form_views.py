...
@company_login
def client_treatment_form(request, domain_name = None, client_id = None, form_id = None):
    ...
    try:
        ...
        if client_treatment_agreement:
            return redirect(reverse('company_client_treatment_form_completed', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id':form_id, 'treat_id': client_treatment_agreement[0].id}))
        else:
            if request.POST:
                ...
                if form.is_valid():
                    ...
                    a=CompanyTreatmentForm.objects.create(client = client_name,
                    ...
                    return render_to_response('company/client_treatment_form.html', locals(), context_instance=RequestContext(request))   
            else:
...
@company_login
def client_treatment_form_completed(request, domain_name = None, client_id = None,  form_id = None, treat_id = None):
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('company/client_treatment_form_completed.html', locals(), context_instance=RequestContext(request))   
        else:
            return render_to_response('company/client_treatment_form_completed.html', locals(), context_instance=RequestContext(request))   
