...
from vcg.admin_management.models import Client, Session, CompanyModules, CreateQuestion, CreateQuestionChoice, CreateQuestionSlider
...
def client_treatment_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('company/client_treatment_list.html', locals(), context_instance=RequestContext(request))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request,(_("Treatment added Successfully")),
                                         fail_silently = True)
            return redirect(reverse('company_client_treatment_list', kwargs={'domain_name': domain_name, 'client_id':client_id }))
        else:
            return render_to_response('company/client_treatment_add.html', locals(), context_instance = RequestContext(request))
    else:
...
def client_treatment_view(request, domain_name = None, client_id = None, module_id = None):
    CompanyClientTreatment.objects.filter(id = module_id).update(question_post_read_status = False)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(request,(_("Your Feedback Sent Successfully")),
                                             fail_silently = True)
                return redirect(reverse('company_client_treatment_view', kwargs={'domain_name': domain_name, 'client_id':client_id, 'module_id':module_id }))
            else:
                return render_to_response('company/client_treatment_view.html', locals(), context_instance = RequestContext(request))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('company/client_treatment_edit.html', locals(), context_instance = RequestContext(request))
        else:
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response('company/client_treatment_custom.html', locals(), context_instance = RequestContext(request))
        else:
...
def client_treatment_session(request, domain_name = None, client_id = None, session_id = None):
    ...
    try:
        ...
        ClientTreatmentSessions.objects.filter(id = session_id).update(post_read_status = False)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(request,(_("Reply Sent Successfully")),
                                             fail_silently = True)
                return redirect(reverse('company_client_treatment_view', kwargs={'domain_name': domain_name, 'client_id':client_id, 'module_id':session.client_treatment.id }))
            else:
                return render_to_response('company/client_treatment_session.html', locals(), context_instance = RequestContext(request))
        else:
...
def client_session_delete(request, domain_name = None, client_id = None, session_id = None):
    ...
    return redirect(reverse('company_client_treatment_view', kwargs={'domain_name': domain_name, 'client_id':client_id, 'module_id':module_name.client_treatment.id }))
...
def client_session_choices(request, domain_name = None, module_id = None, client_id = None, treat_mod_id = None):
    ...
    treatment_module = CompanyModules.objects.get(modules = module_id, company__company_number = request.user.username)
