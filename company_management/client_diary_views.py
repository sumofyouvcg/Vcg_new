...
def client_diary_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('company/client_diary_list.html', locals(), context_instance=RequestContext(request))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(request,(_("Diary  Assigned Successfully")),
                                         fail_silently = True)
            return redirect(reverse('company_client_diary_list', kwargs={'domain_name': domain_name, 'client_id':client_id }))
        else:
            return render_to_response('company/client_diary_add.html', locals(), context_instance = RequestContext(request))
    else:
...
def client_diary_show(request, domain_name = None, client_id = None, diary_id = None):   
    client = Client.objects.get(id = client_id)
    client_user = User.objects.get(username = client.client_number)
    CompanyClientDiary.objects.filter(id = diary_id).update(post_read_status = False)
    try:
                    ...
                    client_events += """
                                 {
                                    ...
                                    url:'/"""+str(domain_name)+"""/company_management/client_diary_view/"""+str(client_id)+"""/"""+str(diary.id)+"""/"""+str(single_date)+"""',
                        ...
                        client_events += """
                                     {
                                        title: '"""+str(diary)+"""',
                                        start: new Date("""+str(single_date.year)+""","""+str((single_date.month)-1)+""","""+str(single_date.day)+"""),
                                        url:'/"""+str(domain_name)+"""/company_management/client_diary_today/"""+str(client_id)+"""/"""+str(diary.id)+"""/"""+str(single_date)+"""',
        ...
        return render_to_response('company/client_diary_show.html', locals(), context_instance = RequestContext(request))
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return redirect(reverse('company_client_diary_show', kwargs={'domain_name': domain_name, 'client_id':client_id, 'diary_id':diary_id }))            
            else:
                return render_to_response('company/client_diary_view.html', locals(), context_instance = RequestContext(request))
...
def client_diary_today(request, domain_name = None, client_id = None, diary_id = None, current_date = None):   
    ...
    try:
        ...
        return render_to_response('company/client_diary_today.html', locals(), context_instance = RequestContext(request))
    except CompanyClientDiary.DoesNotExist:
...
def client_diary_edit(request, domain_name = None, client_id = None, diary_id = None):
    ...
    try:
        try_diary = CompanyClientDiary.objects.get(pk = diary_id)
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('company/client_diary_edit.html', locals(), context_instance=RequestContext(request))
        else:
...
def client_diary_delete(request, domain_name = None, client_id = None):
    if request.method == 'POST':
        ...
        if len(company_client_diary_ids) > 1:
            messages.success(request,(_("Selected Diarys Deleted Successfully")),fail_silently = True)
        else:    
