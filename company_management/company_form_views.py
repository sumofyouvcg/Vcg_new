...
def client_forms_list(request, domain_name = None, client_id = None):
    ...
    return render_to_response('company/client_forms_list.html', locals(), context_instance=RequestContext(request))
...
def client_forms_add(request, domain_name = None, client_id = None):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            return render_to_response('company/client_forms_add.html', locals(), context_instance = RequestContext(request))
    else:
    ...
    try:
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                return render_to_response('company/client_forms_edit.html', locals(), context_instance=RequestContext(request))
        else:
...
def client_forms_view(request, domain_name = None, client_id = None, form_id = None):
    ...
    try:
        ...
        if selected_form.form == 'Intake Form':
            return redirect(reverse('company_client_intake_form', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id' : form_id }))
        elif selected_form.form == 'Treatment Agreement Form':
            return redirect(reverse('company_client_treatment_agreement_form', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id' : form_id }))
        elif selected_form.form == 'Treatment Form':
            return redirect(reverse('company_client_treatment_form', kwargs={'domain_name': domain_name, 'client_id':client_id, 'form_id' : form_id }))
        else:
            return redirect(reverse('company_client_forms_list', kwargs={'domain_name': domain_name, 'client_id':client_id  }))
    except CompanyForms.DoesNotExist:
        messages.success(request,(_('This Form has been removed by Admin')), fail_silently= True)
