...
def get_thumb_filename(file_name):
    ...
    """
    # Changes
    if os.path.splitext(file_name)[1] == '.swf':
...
def create_thumbnail(filename):
    ...
    from django.conf import settings 
    if pattern == '.swf':
        filename = os.path.join(settings.STATICFILES_DIRS[0],'img','photo.jpg')
...
def upload(request):
    ...
    out.close()
    # changes
    if upload_ext in ('.jpeg', '.jpg', '.png', '.gif','.swf' ):
        create_thumbnail(upload_filename)
        url = get_media_url(upload_filename)
    else:
        url = ''
        
    url = url.replace("\\", "/") # Change by Siva
    return HttpResponse("""
...
def get_image_browse_urls(user=None):
    ...
    for filename in get_image_files(user=user):
        images.append({
            'thumb': get_media_url(get_thumb_filename(filename)).replace("\\", "/"), # Change by Siva
            'src': get_media_url(filename).replace("\\", "/") # Change by Siva
        })
    return images
