...
def chat_user(request, domain_name = None):
    ...
    state=''
    return render_to_response('client/client_chat.html', locals(), context_instance=RequestContext(request))
...
def get(request, domain_name = None):
    ...
    if request.method == 'GET':
        ...
        chat_list_today = ChatMessage.objects.filter(Q(sender=request.user, receiver = request.user)|Q(receiver = request.user), received_at__gte=datetime.date.today(), is_read = True).order_by('received_at')
    ...
    client_user = Client.objects.filter(client_number = request.user)
    client_caregivers = []
    client_caregivers_list = ClientCaregivers.objects.filter(client__client_number = request.user, caregiver__role__in = ['Company','Analyst'])
    for client in client_caregivers_list:
        client_caregivers.append(client.caregiver.caregiver_number)
    last_active_list = LastActive.objects.filter(Q(user__username__in = client_caregivers)|Q(user__username = client_user[0].company.company_number), received_at__gt = datetime.datetime.now() - datetime.timedelta(seconds=29090))
...
@client_login
def chat_details(request, domain_name = None):
    ...
    return render_to_response('client/chat_details.html', locals(), context_instance = RequestContext(request))

@client_login
def chat_persons(request, domain_name = None, date = None):
    message_list = Messages.objects.filter(recipient = request.user, delete_status = False).order_by('-modified_at')
    new_messages = Messages.objects.filter(recipient = request.user, read_status = False, delete_status = False)
    ...
    return render_to_response('client/chat_persons.html', locals(), context_instance = RequestContext(request))

@client_login
def chat_history(request, domain_name = None, date = None, user_id = None):
    message_list = Messages.objects.filter(recipient = request.user, delete_status = False).order_by('-modified_at')
    new_messages = Messages.objects.filter(recipient = request.user, read_status = False, delete_status = False)
    ...
    return render_to_response('client/chat_history.html', locals(), context_instance = RequestContext(request))